...
import { CreateBin, newCreateBin } from '@/models/bin';
...
const bin = reactive<CreateBin>(newCreateBin());
...
const submit = async () => {
  if (valid1.value && valid2.value) {
    ...
    const data = new Blob([JSON.stringify(decamelizeKeys(bin))], {
